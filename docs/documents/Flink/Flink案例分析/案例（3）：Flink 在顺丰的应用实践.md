...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_0.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_1.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_2.png)

## 1. 批流统一的实时数仓

建设批流统一的实时数仓可以分为以下 3 个阶段：

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_3.png)

### 1.1 统一数仓规范

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_4.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_5.png)

### 1.2 统一元数据

基于以上统一的数仓规范和层级划分模型，可以将实时和离线的元数据进行统一管理。下游的数据治理过程，比如数据字典、数据血缘、数据质量、权限管理等都可以达到统一。这种统一可以沉淀实时数仓的建设成果，使数仓能更好的落地实施。

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_6.png)

### 1.3 基于 SQL 统一开发流程

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_7.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_8.png)

- Kappa 架构

为了应对 Lambda 架构的缺陷，Jay Kreps 提出了 Kappa 架构，Kappa 架构移除了原有的离线部分，使用纯流式引擎开发。 Kappa 架构的最大问题是，流数据重放处理时的吞吐能力达不到批处理的级别，导致重放时产生一定的延时。

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_9.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_10.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_11.png)

Hudi 的关键特性包括：可回溯历史数据，支持在大规模数据集中根据主键更新删除数据；支持数据增量消费；支持 HDFS 小文件压缩。这些特性恰好能满足我们的需求。

### 2.2 引入 Hudi 加速宽表

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_12.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_13.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_14.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_15.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_16.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_17.png)

### 2.1 实时数据接入

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_18.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_19.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_20.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_21.png)

基于实时数据的统一接入，并将其与现有的离线数仓结合，我们构建了数据资产管理体系。包括规范数仓标准，统一管理元数据，提升数据质量，保障数据安全，盘点数据资产。

## 3. 实时计算平台架构

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_22.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_23.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_24.png)

实时计算平台同样提供 SQL 开发模式，支持手动建表，根据元数据自动识别表及设置表属性。支持创建 UDF、自动识别 UDF、执行 DML 等。

### 3.3 任务管控

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_25.png)

在任务管控方面，实时计算平台尽量简化任务的配置，屏蔽了一些复杂的配置。用户开发完成之后，只需要选择集群，填写资源，就能将任务提交到集群中运行。对每个任务，平台还提供了历史版本控制能力。

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_26.png)

当用户操作任务时，平台会自动解析任务的配置，根据不同的组件提供不同的选项。比如选择了 Kafka 数据源，启动的时候，可以选择从上次消费位置、最早位置、最新位置或指定位置启动。

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_27.png)

任务恢复方面，用户可以选择从 Savepoint 启动已停止的 Flink 任务，便于快速恢复历史状态。

### 3.4 任务运维

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_28.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_29.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_30.png)
...
![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_31.png)

首先，希望能够支持更多 SQL 的语法和特性，支持更多可用的连接器，以及实现 SQL 任务的自动调优等。

## 2. 精细化资源管理

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_32.png)

其次，基于 Flink On Kubernets 、任务的自动弹性扩缩容，Task 级别的细粒度资源调度实现精细化的资源调度管理，使得 Flink 任务达到全面的弹性化和云原生化。

## 3. 流批一体

![](../../assets/images/Flink/attachments/案例（3）：Flink在顺丰的应用实践_image_33.png)
