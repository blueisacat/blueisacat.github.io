...
## 1.3 表的属性
在 `CREATE TABLE` 语句中，可以指定表的属性。比如，存储桶数量(buckets)、存储介质(storage_medium)、复制数量(replication_num)和冷热存储策略(storage_policy)。
...
* 方法2：可以使用 `show tablet from tableName` 命令查看表中每个tablet的大小，由 `RemoteDataSize` 字段表示。
