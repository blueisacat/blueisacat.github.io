...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_0.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_1.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_2.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_3.png)

在 ~/logs/csp/sentinel-record.log 日志文件中将打印如下信息：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_4.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_5.png)

现在我们来创建一个 Token Server ，我启动了三个应用实例，其中 8720 端口对应的实例为 token server，选择 192.168.0.104:8720 这台为服务端，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_6.png)

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_7.png)

保存后推送，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_8.png)

页面上机器的显示方式为 ip@commandPort，其中 commandPort 为应用端暴露给 Sentinel 控制台的端口。

选择好以后，点击【保存】按钮，刷新页面即可以看到 token server 分配成功：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_9.png)

并且我们可以在页面上查看 token server 的连接情况，点击【连接详情】即可查看，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_10.png)

我们可以在【集群流控】页面的【Cluster Client列表】中查看具体的集群客户端，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_11.png)

### 查看效果

通过 jmeter 模拟流量同时请求两台客户端机器，过一段时间后观察效果。

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_12.png)

然后在监控页面看到对应资源的集群维度的总 qps ，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_13.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_14.png)

看下每个文件中具体的内容：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_15.png)

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_16.png)

可以看到同一秒两个 client 通过的 qps 相加的结果是保持在10以下的。

那按照道理 dashboard 中是聚合的两个 client 的 qps 总和，不应该超过10才对，经过与 Sentinel 的开发人员 

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_17.png)

从结果中发现统计结果确实聚合了三台机器的统计值，再把 dashboard 请求 metrics 的结果打印出来看一下：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_18.png)
...
经过乐有的指导，发现可能是我在同一台机器中同时起了三个应用名相同的进程，而如果在本地启动多个同名应用时，需要加入 

现在我把每个应用上都加上 -Dcsp.sentinel.log.use.pid=true 的参数，再次模拟该请求，观察一下实时监控的结果，发现如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_19.png)

再看 metrics 日志文件，发现文件名也带上了进程号：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_20.png)

再看下每个文件中的实际统计结果：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_21.png)

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_22.png)

再看 MetricFetcher 中打印的日志，发现也没有再去请求 token-server 的 metric 了，如下图所示：

![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_23.png)
...
![](../../assets/images/Sentinel/attachments/Sentinel实战（5）：集群限流环境搭建_image_24.png)
